...
const resources = [
  ['Assignment','Unit 4 Mock IRB Packet Instructions','Directions for preparing the Draft IRB Packet Review / Mock IRB Submission Package.','resources/Assignment_4_Instructions.docx'],
  ['Rubric','Unit 4 Mock IRB Packet Rubric','Rubric criteria for completeness, proposed pathway/protocol accuracy, alignment, protections, participant-facing materials, instruments, administrative readiness, and presentation.','resources/Unit_4_Assignment_Rubric.docx'],
  ['Checklist','IRB Application Package Checklist','Student-facing pathway-specific packet inventory and final alignment check.','resources/IRB_Application_Package_Checklist.docx'],
  ['Job Aid','Submitting the PG IRB Application Form','Step-by-step guide for official submission and saving/uploading the confirmation email.','resources/Job_Aid_Submitting_PG_IRB_Application_Form_and_Uploading_Confirmation.docx'],
  ['Guide','PG IRB Investigator Guide','Official investigator guidance for proposed pathway self-assessment, required forms, training, and submission.','resources/PG_IRB_Investigator_Guide.pdf'],
  ['Training','CITI Training Quick Reference','Student support for completing the required human-subjects research training.','resources/CITI_Training.pdf'],
  ['Training','Human Subjects Research Training Requirements','Current institutional training options and requirements.','resources/Human_Subjects_Research_Training_Requirements.docx'],
  ['Worksheet','IRB-310 Human Research Determination','Self-assessment worksheet used to understand whether an activity may constitute human research.','resources/IRB-310_Human_Research_Determination.docx'],
  ['Worksheet','IRB-312 Exemption Determination','Current exemption-determination worksheet for proposed pathway planning.','resources/IRB-312_Exemption_Determination.docx'],
  ['Worksheet','IRB-313 Expedited Review','Current expedited-review worksheet for proposed pathway planning.','resources/IRB-313_Expedited_Review.docx'],
  ['Protocol','IRB-503b Human Research Determination','Current Human Research Determination protocol template.','resources/IRB-503b_Human_Research_Determination_Protocol.docx'],
  ['Protocol','IRB-503c Exempt Study Protocol','Current Exempt Research protocol template.','resources/IRB-503c_Exempt_Study_Protocol.docx'],
  ['Protocol','IRB-503a Expedited Research Protocol','Current Expedited Research protocol template.','resources/IRB-503a_Expedited_Research_Protocol.docx'],
  ['Template','IRB-502b Exempt Information Sheet','Research Participant Information Sheet for exempt research when participants are contacted.','resources/IRB-502b_Exempt_Research_Information_Sheet.docx'],
  ['Template','IRB-502a Expedited Informed Consent','Research Participant Informed Consent Form for expedited research.','resources/IRB-502a_Expedited_Informed_Consent_Form.docx']
];

function defaultState(){
  return {status:'planning',pathway:'none',checks:{},matrix:{},times:{consent:0,survey:0,interview:0,followup:0,other:0}};
}
function loadState(){
...
function checkKey(group,id){ return `${group}.${id}`; }
function isChecked(group,id){ return !!state.checks[checkKey(group,id)]; }
function setChecked(group,id,value){ state.checks[checkKey(group,id)] = value; save(); }
...
function renderPathway(){
  ...
  card.innerHTML=`<div class="pathway-main"><span class="tag">PROPOSED PATHWAY</span><h3>${info.label}</h3><p><strong>Protocol:</strong> ${info.protocol}</p><p>${info.note}</p></div><div><span class="tag">PARTICIPANT-FACING REQUIREMENT</span><h3>What to prepare</h3><p>${info.participant}</p></div>`;
}

function groupHtml(group, groupName='packet'){
  return `<article class="check-group"><h3>${group.title}</h3>${group.note?`<p class="group-note">${group.note}</p>`:''}${group.items.map((item,i)=>`<label class="check-item"><input type="checkbox" data-check-group="${groupName}.${group.id}" data-check-index="${i}" ${isChecked(`${groupName}.${group.id}`,i)?'checked':''}><span>${item}</span></label>`).join('')}<div class="check-progress" data-progress-group="${groupName}.${group.id}" data-progress-total="${group.items.length}"></div></article>`;
}

function renderPacket(){
  document.getElementById('packet-checks').innerHTML=packetGroups.map(g=>groupHtml(g,'packet')).join('');
}
function renderSite(){
  document.getElementById('site-checks').innerHTML=siteGroups.map(g=>groupHtml(g,'site')).join('');
}
function renderParticipantGroups(){
  document.getElementById('participant-groups').innerHTML=participantGroups.map(g=>`<details open><summary>${g.title}</summary><div class="accordion-content">${g.items.map((item,i)=>`<label class="check-item"><input type="checkbox" data-check-group="participant.${g.id}" data-check-index="${i}" ${isChecked(`participant.${g.id}`,i)?'checked':''}><span>${item}</span></label>`).join('')}<div class="check-progress" data-progress-group="participant.${g.id}" data-progress-total="${g.items.length}"></div></div></details>`).join('');
}
function renderPreflight(){
  document.getElementById('preflight-checks').innerHTML=preflightGroups.map(g=>`<article class="preflight-card"><h3>${g.title}</h3>${g.items.map((item,i)=>`<label class="check-item"><input type="checkbox" data-check-group="preflight.${g.id}" data-check-index="${i}" ${isChecked(`preflight.${g.id}`,i)?'checked':''}><span>${item}</span></label>`).join('')}<div class="check-progress" data-progress-group="preflight.${g.id}" data-progress-total="${g.items.length}"></div></article>`).join('');
}

function renderMatrix(){
  const head=matrixCols.map(([,label])=>`<th>${label}</th>`).join('');
  const rows=alignmentRows.map(row=>{
    const cells=matrixCols.map(([col])=>{
      if(!row.cols.includes(col)) return '<td><span class="na">N/A</span></td>';
      const key=`${row.id}.${col}`;
      return `<td><input type="checkbox" aria-label="${row.label} aligned in ${col}" data-matrix="${key}" ${state.matrix[key]?'checked':''}></td>`;
    }).join('');
    return `<tr><td>${row.label}</td>${cells}</tr>`;
  }).join('');
  document.getElementById('alignment-matrix').innerHTML=`<table class="matrix"><thead><tr><th>Study element</th>${head}</tr></thead><tbody>${rows}</tbody></table>`;
}

function renderResources(){
  document.getElementById('resource-grid').innerHTML=resources.map(([type,title,desc,url])=>`<article class="resource-card"><span>${type}</span><h3>${title}</h3><p>${desc}</p><a href="${url}" target="_blank" rel="noopener">Open resource →</a></article>`).join('');
}

function wireChecks(){
  document.querySelectorAll('[data-check-group]').forEach(cb=>cb.addEventListener('change',e=>{
    setChecked(e.target.dataset.checkGroup,e.target.dataset.checkIndex,e.target.checked);
  }));
  document.querySelectorAll('[data-matrix]').forEach(cb=>cb.addEventListener('change',e=>{
    state.matrix[e.target.dataset.matrix]=e.target.checked; save();
  }));
}
...
function groupCount(prefix,g){
  const done=g.items.filter((_,i)=>isChecked(`${prefix}.${g.id}`,i)).length;
  return {done,total:g.items.length};
}
...
function updateGroupProgress(){
  document.querySelectorAll('[data-progress-group]').forEach(el=>{
    ...
    let done=0;
    for(let i=0;i<total;i++) if(isChecked(group,i)) done++;
    el.textContent=`${done} of ${total} checked`;
  });
  const packetTotal=packetGroups.reduce((n,g)=>n+g.items.length,0);
  const packetDone=packetGroups.reduce((n,g)=>n+groupCount('packet',g).done,0);
  document.getElementById('packet-count').textContent=`${packetDone} of ${packetTotal} checked`;
  document.getElementById('packet-bar').style.width=`${packetTotal?packetDone/packetTotal*100:0}%`;
  const siteTotal=siteGroups.reduce((n,g)=>n+g.items.length,0);
  const siteDone=siteGroups.reduce((n,g)=>n+groupCount('site',g).done,0);
  document.getElementById('site-count').textContent=`${siteDone} of ${siteTotal} checked`;
  document.getElementById('site-bar').style.width=`${siteTotal?siteDone/siteTotal*100:0}%`;
}

function updateAlignment(){
  const applicable=alignmentRows.reduce((n,row)=>n+row.cols.length,0);
  const done=Object.values(state.matrix).filter(Boolean).length;
  const missing=Math.max(0,applicable-done);
  const el=document.getElementById('alignment-result');
  if(done===0){
    el.innerHTML='<strong>Alignment matrix not started.</strong> Check a box only after you have compared that study element across the indicated document.';
  } else if(missing===0){
    el.innerHTML='<strong>All matrix comparisons are marked reviewed.</strong> This means you completed the self-check—not that the packet is officially aligned or approved. Student verification and faculty/RPC/IRB review are still required.';
  } else {
    el.innerHTML=`<strong>${done} of ${applicable} applicable comparisons marked reviewed.</strong> ${missing} comparison${missing===1?' remains':'s remain'} to check.`;
  }
}

function updateTime(){
  const ids=['consent','survey','interview','followup','other'];
  let total=0;
  ids.forEach(id=>{
  ...
  if(total<60) result.innerHTML=`<strong>${total} minute${total===1?'':'s'}</strong><span>Estimated participant time</span>`;
  else {
    const h=Math.floor(total/60), m=total%60;
    result.innerHTML=`<strong>${h} hr${h===1?'':'s'}${m?` ${m} min`:''}</strong><span>Estimated participant time (${total} minutes)</span>`;
  }
}
...
function updateSummary(){
  ...
  categoryMap.forEach(([label,arr,prefix])=>{
    ...
    const done=arr.reduce((n,g)=>n+groupCount(prefix,g).done,0);
    const remaining=total-done;
    cards.push(`<article class="summary-card ${remaining===0?'complete':'attention'}"><span>${label}</span><strong>${done}/${total}</strong><p>${remaining===0?'All self-check items marked reviewed.':`${remaining} item${remaining===1?'':'s'} still unmarked.`}</p></article>`);
  });
  const matrixTotal=alignmentRows.reduce((n,row)=>n+row.cols.length,0);
  const matrixDone=Object.values(state.matrix).filter(Boolean).length;
  cards.push(`<article class="summary-card ${matrixDone===matrixTotal?'complete':'attention'}"><span>Alignment matrix</span><strong>${matrixDone}/${matrixTotal}</strong><p>${matrixTotal-matrixDone===0?'All applicable comparisons marked reviewed.':`${matrixTotal-matrixDone} comparison${matrixTotal-matrixDone===1?'':'s'} still unmarked.`}</p></article>`);
  const p=pathwayInfo[state.pathway];
  cards.push(`<article class="summary-card ${p?'complete':'attention'}"><span>Proposed pathway</span><strong>${p?'Selected':'Not set'}</strong><p>${p?p.label:'Select the proposed pathway you are preparing.'}</p></article>`);
  const status=statusMessages[state.status];
  cards.push(`<article class="summary-card complete"><span>Current status</span><strong>${state.status==='approved'?'IRB Approved':'Recorded'}</strong><p>${status.title}</p></article>`);
  cards.push(`<article class="summary-card attention"><span>Important</span><strong>Not approval</strong><p>This completion summary is a self-check only and cannot authorize IRB submission or research activity.</p></article>`);
  document.getElementById('summary-board').innerHTML=cards.join('');
}

function updateOverall(){
  const groups=allCheckGroups();
  const checklistTotal=groups.reduce((n,[,g])=>n+g.items.length,0);
  const checklistDone=groups.reduce((n,[prefix,g])=>n+groupCount(prefix,g).done,0);
  const matrixTotal=alignmentRows.reduce((n,row)=>n+row.cols.length,0);
  const matrixDone=Object.values(state.matrix).filter(Boolean).length;
...
function printSummary(){
  ...
  const sections=groups.map(([prefix,g])=>{
    const rows=g.items.map((item,i)=>`<li>${isChecked(`${prefix}.${g.id}`,i)?'☑':'☐'} ${item}</li>`).join('');
    return `<section><h2>${g.title}</h2><ul>${rows}</ul></section>`;
  }).join('');
  const matrixRows=alignmentRows.map(row=>{
    const checked=row.cols.filter(col=>state.matrix[`${row.id}.${col}`]).length;
    return `<tr><td>${row.label}</td><td>${checked} / ${row.cols.length}</td></tr>`;
  }).join('');
  const p=pathwayInfo[state.pathway];
  const totalTime=Object.values(state.times).reduce((n,v)=>n+(Number(v)||0),0);
  const w=window.open('','_blank','width=1000,height=800');
  if(!w){window.print();return;}
  w.document.write(`<!doctype html><html><head><title>ED825 IRB Readiness Summary</title><style>body{font-family:Arial,sans-serif;margin:36px;color:#111;line-height:1.45}h1{margin-bottom:4px}.notice{border:2px solid #a00;padding:12px;margin:18px 0}.meta{background:#f3f3f3;padding:12px}.meta p{margin:4px 0}section{break-inside:avoid;margin:20px 0}ul{padding-left:22px}li{margin:5px 0}table{border-collapse:collapse;width:100%}th,td{border:1px solid #bbb;padding:7px;text-align:left}</style></head><body><h1>ED825 IRB Readiness Summary</h1><p>Supplemental student self-check</p><div class="notice"><strong>This is not IRB approval, an official pathway determination, or authorization to begin research.</strong></div><div class="meta"><p><strong>Current status:</strong> ${statusMessages[state.status].title}</p><p><strong>Proposed pathway:</strong> ${p?p.label:'Not selected'}</p><p><strong>Estimated participant time:</strong> ${totalTime} minutes</p></div>${sections}<section><h2>Alignment Matrix Review</h2><table><thead><tr><th>Study element</th><th>Comparisons marked reviewed</th></tr></thead><tbody>${matrixRows}</tbody></table></section><p><strong>Final reminder:</strong> Independently verify the current course and PG IRB requirements and consult faculty/RPC when uncertain.</p></body></html>`);
  w.document.close(); w.focus(); setTimeout(()=>w.print(),250);
}

function wireEvents(){
  document.getElementById('student-status').addEventListener('change',e=>{state.status=e.target.value;save();renderStatus();});
  document.getElementById('pathway-select').addEventListener('change',e=>{state.pathway=e.target.value;save();renderPathway();});
  ['consent','survey','interview','followup','other'].forEach(id=>{
    const input=document.getElementById(`time-${id}`);
    input.value=state.times[id]||0;
    input.addEventListener('input',updateTime);
  });
  document.getElementById('print-summary').addEventListener('click',printSummary);
  document.getElementById('clear-progress').addEventListener('click',()=>{
    if(confirm('Clear all ED825 readiness progress saved by this site in this browser?')){
...
function renderAll(){
  renderStatus(); renderPathway(); renderPacket(); renderSite(); renderParticipantGroups(); renderPreflight(); renderMatrix(); renderResources(); wireChecks(); updateTime(); updateAllProgress();
}
